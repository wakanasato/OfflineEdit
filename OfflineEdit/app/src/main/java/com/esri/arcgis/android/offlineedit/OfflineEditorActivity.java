...
/**
 * Created by ej2238 on 2016/10/05.
 */
...
public class OfflineEditorActivity extends Activity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        // ライセンス認証
        LicenseResult licenseResult = ArcGISRuntime.setClientId("SorBouaySCqTebjp");
        ...
        if (licenseResult == LicenseResult.VALID && licenseLevel == LicenseLevel.BASIC) {
            ...
            Toast.makeText(this, "invalid licence", Toast.LENGTH_LONG).show();
        }
         ...
         * Layer
         *
         * 初めてインストールしたときには、geodatabase含む複数ファイルが作成される
         *
         */
        // ベースマップを追加
    ...
    public void save() {
        ...
        if (editingmode == POINT)
            try {
                ...
                getTemplatePicker().getSelectedLayer();


                ((FeatureLayer) mapView
                        .getLayerByID(getTemplatePicker()
                        .getSelectedLayer()
                        .getID()))
                        .getFeatureTable()
    ...
    public class ConnectToServer extends AsyncTask<String, Void, Void> {
        ...
        // UIスレッドとして最初に呼ばれる：なのでプログレスバーの処理
        @Override
        ...
        // 非同期なので別スレッドで裏実行：AsyncTask<String, Void, Void>のString部分がparamsになるのかな。
        @Override
